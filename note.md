...
### -pedantic を使う理由（42 C++）
- 目的: C++98純度の確保、可搬性、一貫した採点、学習効果
- 効果: 標準外の構文や拡張使用を警告/拒否（-Werror併用でビルド失敗に）
- 代表例:
  - `long long` の使用（C++98では拡張）
  - 可変長配列（VLA）や GNU拡張構文（statement expression など）
- 運用:
  - 通常は `-std=c++98 -Wall -Wextra -Werror -pedantic`
  - 調査時だけ `-Wpedantic` に落として警告を洗い出し、修正後に戻す
